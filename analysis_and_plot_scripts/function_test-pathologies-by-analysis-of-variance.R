...
# Background ----
# The purpose of this script is to test for significant differences between our different pathologies in the ear tag data
...
test_mean_perc_meth_by_ANOVA <- function(input_df) {
  test_data <- input_df %>%
    group_by(pathology) %>%
    filter(n() >= 2) %>%
    ungroup()
  ...
  linear_model_perc_meth <- lm(mean_perc_meth_wo_KUNE ~ pathology,
                               data = select_target_to_plot)
  shapiroTest <- shapiro_test(linear_model_perc_meth$residuals)
  LevenesTest <- select_target_to_plot %>%
    mutate(pathology = factor(pathology)) %>%
    levene_test(mean_perc_meth_wo_KUNE ~ pathology)
  if (shapiroTest$p.value >= 0.05 & LevenesTest$p >= 0.05) {
    normality <- TRUE
    equal_variances <- TRUE
    test_results <- test_data %>%
      anova_test(mean_perc_meth_wo_KUNE ~ pathology)
    method = "ANOVA"
  } else if (shapiroTest$p.value >= 0.05 & LevenesTest$p <= 0.05) {
    normality <- TRUE
    equal_variances <- FALSE
    test_results <- test_data %>%
      welch_anova_test(mean_perc_meth_wo_KUNE ~ pathology)
    method = "Welchs_ANOVA"
  } else if (shapiroTest$p.value <= 0.05 & LevenesTest$p >= 0.05) {
    normality <- FALSE
    equal_variances <- TRUE
    test_results <- test_data %>%
      kruskal_test(mean_perc_meth_wo_KUNE ~ pathology)
